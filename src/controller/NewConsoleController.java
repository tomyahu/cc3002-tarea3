 ...
 *
 */
public class NewConsoleController implements IController {
   ...
   * @param viewand
   */
  public NewConsoleController(IGameLogic game, ConsoleView view) {
    this.view = view;
    this.game = game;
  ...
  public int AskForCardFromHand(IPlayer player) {
    ...
    while (num < 0 || num >= player.getHandSize()) {
      view.showMessage(
          "Por favor, ingresar un numero entre el 0 y el " + (player.getHandSize()-1) + ".");
      num = in.nextInt();
    }
    return num;
  }
